...
$("#del_user").click(function (event) {
    ...
    var userid = str.find('input[name=e_id]').val(); 
...
$("#edit_info").click(function (event) {
    ...
    location.href='./get_edit?id='+userid;
    ...
    var title_tab = '<div class="active" id="tabs_edit' + userid + '" onclick="changetabs(\'edit' + userid + '\')" temp="tabs"><i class="icon-edit"></i>  编辑用户:<span class="icon-remove" onclick="closetabs(\'edit' + userid + '\')"></span></div>';
    $.ajax({
        url: './get_edit?id=' + userid,
        success: function (jsoan) {
            $("#content_allmainview").append(jsoan);
            $('#tabs_title').append(title_tab);
            changetabs('edit' + userid);
...
function searchUser() {
    ...
    var url = encodeURI('./searchUser?keyword=' + keyword + '&pagesize=' + $("#pagesize").val()+'&key='+key);
    $.ajax({
        url: url,
        data: postdata,
        type: 'post',
...
function showSearchResult(json) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        trs += '<td width="40">';
        trs += i + 1;
        trs += '    <input type="hidden" name="e_id" value="' + data[i]['id'] + '">'; 
        trs += '</td> ';
        trs += '<td>';
        trs += data[i]['hapend_date'];
        trs += '</td>';
        trs += '<td>';
        trs += data[i]['jifang'];
        trs += '</td> ';
        ...
        trs += data[i]['chuli'];
        trs += '</td>';
        trs += '<td style="width:100px;word-break:break-all;">';
        trs += data[i]['result'];
        trs += '</td>';
        trs += '<td>';
        trs += data[i]['huifu_date'];
        trs += '</td>'; 
        trs += '</tr>';
    }
    ;
...
$("#shang").click(function (event) {
    ...
    $.ajax({
        url: './searchUser?page=' + (parseInt(page) - 1) + '&pagesize=' + size + keys,
        success: function (json) {
            json = eval("(" + json + ")");
            showSearchResult(json);
        }
...
$("#lastye").click(function (event) {
    ...
    $.ajax({
        url: './searchUser?page=' + maxpage + '&pagesize=' + size + keys,
        success: function (json) {
            json = eval("(" + json + ")");
            showSearchResult(json);
        }
