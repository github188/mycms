...
class SmsController extends Controller{
    ...
    function lists(){
        ...
        //$db=M('blacklist','','intelligent_alarm');
        //$db=M('intelligent_alarm_log');
        $db=D('Alarm');
        $data['pagesize']=15; 
        $res = $db->limit(0,$data['pagesize'])->order("endtime desc")->select();
        $data['total']=$db->count();
    ...
    function deleteInfo(){
        if($_GET['id']){
            ...
            //$dbalarm=M('intelligent_alarm','','Hxtong');
            //$dbalarm=M('intelligent_alarm');
            $dbalarm=D('Alarm');
            $alarm_id = $_GET['id']; 
            $is_exit_alarm =  $dbalarm->where("id='$alarm_id'")->find();
            $send_content='';
            if(!$is_exit_alarm){
                ...
                $dalarm =$dbalarm->where("id='$alarm_id'")->delete(); 
                    if($dalarm){
                        ...
                        $update['operate_ip']=$this->getClientIp();
                        $update['type'] =$is_exit_alarm['type'];
                        $update['strtime'] =$is_exit_alarm['strtime'];
                        $update['endtime'] =$is_exit_alarm['endtime'];
                        $update['keywords'] =$is_exit_alarm['keywords']; 
                        $log->add($update);
    ...
    function searchInfo(){
        if($_GET['pagesize']){
            ...
            $res = $db->where($where)->limit($start,$end)->order("endtime desc")->select();
    ...
    function get_edit(){
        if($_GET['id']){
            $alarm_id   = $_GET['id'];
            $alarmdb =  D("Alarm");
            //$alarmdb =M('intelligent_alarm','','Hxtong');
            $logdb = M("intelligent_alarm_log");
            $alarmres = $alarmdb->where("id='$alarm_id'")->find();
    ...
    //执行修改
    function doEdit(){
        if($_POST&&$_POST['log_id']){
            $alarm_id = $_POST['log_id'];
            //$log_id = $_POST['log_id'];
            $type = trim($_POST['type']);
            if(!in_array($type,array('GTP','TongEASY','MQ'))){
                echo json_encode(array("code"=>10001,"msg"=>"请选择正确的的中间件！"));
    ...
    */
    function fhUser(){
        $db=M("user");
        $file = "./public/temp/codes.txt";
        $fhs = explode("\n",file_get_contents($file));
        $code = 0;
        foreach($fhs as $v){ 
            $row = explode("|",str_replace("01","|",$v));
            //dump($row);
            $row1 = explode("_",$row[0]);
            if($code==$row1[1]){
                continue;
            }
            echo $code = $row1[1];
            ...
            $data['username'] = $code;
            $data['nickname'] = $fhname;
            $data['reg_date'] = date("Y-m-d H:i:s");
            $data['password'] = md5(md5($code) . "xtszhgl");
