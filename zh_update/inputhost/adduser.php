...
if ($_POST) {
    ...
    $contadd = "<?php" . "\r\n";
    $contadd .="\$userlist=array(" . "\r\n";
    foreach ($userlist as $k => $v) {
        $contadd .="    '" . $k . "'=>'" . $v . "'," . "\r\n";
    }
    $contadd .="    '" . $_POST["username"] . "'=>'" . $_POST["newpassword2"] . "'," . "\r\n";
    $contadd .=");";
    $handle = fopen("config.php", "w") or die("10001");
    if (fwrite($handle, $contadd)) {
        echo "10000";
    }
    fclose($handle);
    exit();
                        ...
                        <div class="tables"> 
                            <table width="525" height="240" id="formtable"> 
                                <tr>
                                    ...
                                    <td colspan="4">  
                                        <input value="" onblur="onblure(this);" class="text vam" type="text" name="username" id="oldpassword">
        ...
        <script>
            ...
            function onblure(e) {
                ...
                if (old) {
                    ...
                    var data = {"username": old};
                    $.ajax({
                        url: "adduser.php?action=check",
                        data: data,
                        type: "post",
            ...
            jQuery(function ($) {
                ...
                $("#submit").click(function () {
                    ...
                    if (newpassword1 !== newpassword2) {
                        $("[name=newpassword2]").focus();
                        $("[name=newpassword2]").css("border-color", "red");
                        $("span#newpassword2").html("两次密码输入不一致");
                        return false;
                    }
                    $.ajax({
                        url: "adduser.php",
                        data: $("[name=adduser]").serialize(),
                        type: "post",
                        success: function (json) {
                            if (json === "10000") {
                                $("[name=username]").val("");
