$(function ($) {
    $("#add-case").click(function (event) {
        location.href='/index.php/Home/Case/add';
    });
    $("#subcase").click(function (event) {
        ...
        var case_class_id = $("select[name=case_class_id]").val();
        if (!case_class_id) {
            $("select[name=case_class_id]").focus();
            $("select[name=case_class_id]").addClass('error');
            return false;
        } else {
            $("input[name=happen_date]").removeClass('error');
        }
        ...
        $.ajax({
            url: '/index.php/Home/Case/doa',
            type: 'post',
            data: $("#upcaseform").serialize(),
            success: function (json) {
                json = eval("(" + json + ")");
                $("#commonmodal .modal-body").html("<h2>" + json.message + "</h2>");
                $("#commonmodal").modal("show");
                if (json.code == "10000") {
                    $("#upcaseform [name=case_id]").val(json.id);
                    $("#commonmodal").on("hidden.bs.modal", function () {
                        location.href = "/index.php/Home/Case/show?id=" + json.id;
                    })
                }
            }
        })
    ...
    $("#addcl").click(function () {
        ...
        adclh += '<input class="form-control" name="case_class_name" value="" placeholder="" type="text"></td>';
        adclh += '</tr> ';
        adclh += '</tbody>';
        adclh += '</table>';
        adclh += '</form>';
        ...
        adclh += '    <button id="case_cs" data-id="' + did + '" class="btn btn-primary" type="button" title="名称不能为空" log="add">';
    ...
    $("body").on("click", "#case_cs", function () {
        var name = $("body").find("[name=case_class_name]").val();
        if (!name) {
            return false;
        }
        var did = $(this).attr("data-id");
        $.ajax({
            url: '/index.php/Home/Case/addcl?name=' + name,
            success: function (json) {
                json = eval("(" + json + ")");
                if (json.code == "10000") {
                    $("#case_class_id").html('');
                    ...
                    for (var i = json.res.length - 1; i >= 0; i--) {
                        if (json.res[i]['case_class_name'] == name) {
                            option += '<option selected value="' + json.res[i]['case_class_id'] + '">' + json.res[i]['case_class_name'] + '</option>';
                        } else {
                            option += '<option value="' + json.res[i]['case_class_id'] + '">' + json.res[i]['case_class_name'] + '</option>';
                        }

                    }
                    ;
                    $("#case_class_id").html(option);
                    $("#" + did).modal("hide");
                }
            }
        })
...
//搜索
    $("#csu").click(function (event) {
        var keyw = $("#caseSearchInput").val();
        if (!keyw) {
            return false;
        }
        location.href = '/index.php/Home/Case/search?k=' + keyw;
        return false;
    });
    $("#caseSearchInput").keydown(function (event) {
        if (event.keyCode == 13) {
            var keyw = $(this).val();
            if (!keyw) {
                return false;
...
//文件上传
    $("#uploadcase").click(function (event) {
        var title = $("input[name=title]").val();
        if (!title) {
            $("input[name=title]").focus();
            $("input[name=title]").addClass('error');
        ...
        var case_class_id = $("select[name=case_class_id]").val();
        if (!case_class_id || case_class_id == 0) {
            $("select[name=case_class_id]").focus();
            $("select[name=case_class_id]").addClass('error');
            layer.msg("请选择分类！");
            return false;
        } else {
            $("input[name=case_class_id]").removeClass('error');
        }
        var keyword = $("input[name=keyword]").val();
        if (!keyword) {
            $("input[name=keyword]").focus();
        ...
        var keyword = $("input[name=casedoc]").val();
        if (!keyword) {
            $("input[name=casedoc]").focus();
            $("input[name=casedoc]").addClass('error');
            layer.msg("请选择word文档！");
            return false;
        } else {
            $("input[name=casedoc]").removeClass('error');
        } 
        $.ajax({
            url: '/index.php/Home/Case/titleCheck',
            type: 'post',
            async:false,
            data: $("#upcaseform").serialize(),
            success: function (json) {
